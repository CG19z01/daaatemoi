...
// Au-delà de cette distance du rivage, toute la case est du même côté : une
// seule vérification suffit. Près de l'eau, chaque position est examinée.
const MARGE_DU_RIVAGE = 250;
...
const poserUnBatiment = (grille, choisirUnBatiment, aleatoire, x, y, aLEau = null) => {
...
export const construireDecor = (fond, positionsDesLieux = [], limiteDeLaVille = LIMITE_PAR_DEFAUT) => {
  ...
  const examinerLaMer = creerTestDeLaMer(fond.littoral);
  const aLEau = (x, y) => examinerLaMer(x, y).aLEau;
  ...
  for (let x = -limiteDeLaVille; x <= limiteDeLaVille; x += PAS_DE_LA_GRILLE) {
    for (let y = -limiteDeLaVille; y <= limiteDeLaVille; y += PAS_DE_LA_GRILLE) {
      const rivage = examinerLaMer(x + PAS_DE_LA_GRILLE / 2, y + PAS_DE_LA_GRILLE / 2);
      ...
      const batiment = poserUnBatiment(
        ...
        x,
        y,
        rivage.distance < MARGE_DU_RIVAGE ? aLEau : null,
      );
      // Dernier contrôle sur la position réellement retenue : loin du rivage,
      // deux morceaux de côte peuvent se disputer le point, et le centre de la
      // case ne suffit alors plus à trancher.
      if (batiment && !aLEau(batiment.x, batiment.y)) lesBatiments.push(batiment);
    }
  }
